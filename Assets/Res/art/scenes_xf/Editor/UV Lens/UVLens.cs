...
public class UVLens : EditorWindow
{
    Mesh mesh;
    ...
    [MenuItem("Window/Tools/UV Lens")]
    static void Open()
    {
        EditorWindow window = GetWindow(typeof(UVLens));
        window.titleContent = new GUIContent("❏ UV Lens");
        window.minSize = Vector2.one * 150;
        ((UVLens)window).ResetMesh();
    }
    ...
    void DrawViewport()
    {
        ...
        {
            GUI.changed = true;
            scale += e.delta.magnitude * Mathf.Sign(Vector2.Dot(e.delta, new Vector2(1, -.1f).normalized)) * (2 / viewport.width) * scale * 6;
        ...
        Matrix4x4 viewMatrix = Matrix4x4.Scale(Vector3.one * scale) * Matrix4x4.Translate(panshift + new Vector2(-.5f, -.5f));
