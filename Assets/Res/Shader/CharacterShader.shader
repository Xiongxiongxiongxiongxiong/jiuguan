...
Shader "XingFei/CharacterShader" {
    Properties {
        ...
        _DyeColor2("DyeColor2",Color) = (1,0,0,1)
        ...
        _CustomLightDir("自定义主光方向",Vector) = (1,-1,-0.5,0)
...
// CharacterShader Very High
    ...
    SubShader {
        ...
        // 现在改用使用截取渲染不透明物体之后的RT:_ScreenCopyTex作为基色的方式，隐身状态仍然Blend Off，那么就不需要这个Pass了
        // Pass {
        ...
        // UsePass "XingFei/Effect/Soul/EffectsPreZWrite"

        

        // normal pass
        // todo: 不用考虑蒙皮的问题，似乎Untiy统一处理了
        // todo：暂时也不用考虑shadow cast的问题，在fallback里写一个标准材质，里面的shadow相关pass会直接继承过来
        // todo: 增加一个emisive 贴图
        // todo: env map 的 IBL 算法调整
        // todo: sss skin效果
        // todo: 接受动态点光照射
        Pass {
            ...
            // 自定义主光方向
            #pragma multi_compile _ _ENABLECUSTOMLIGHTDIR_ON
...
// toon outline pass
        ...
        Pass
        {
            ...
            CGPROGRAM
                ...
                #pragma exclude_renderers vulkan xboxone ps4 psp2 n3ds wiiu
                //#pragma multi_compile _ _OUTLINEEFFECT_ON // 卡通描边效果，不再用关键字的方案，避免内存过大
                #pragma multi_compile _ _ALPHATEST_ON
                // #pragma multi_compile _ _HIGH2LOWBAKE_ON
                
                // #pragma multi_compile _ _SCREENCLIPSWITCH_ON

                struct vertInput {
                    HighPrec4 pos : POSITION;
                    HighPrec3 nor : NORMAL;
                ...
                struct vertOutput {
                    ...
                    HighPrec2 uv  : TEXCOORD0;
                    HighPrec4 testColor : TEXCOORD1;
                };

                // uniform MidPrec _CharacterDoToneMapping; // 默认是0,角色不做ToneMapping

                uniform LowPrec _AnisoEffect;

                uniform LowPrec _OutlineEffect;
                uniform HighPrec _OutlineUseVertexColor;
                uniform MidPrec _OutlineThreshold;
                uniform LowPrec _OutlineLight;
                uniform MidPrec _OutlineMaxThickness;
                uniform MidPrec _OutlineMinThickness;
                uniform HighPrec _OutlineThickness;
                LowPrec _OutlineExpandWithDistance;
                uniform MidPrec4 _OutlineColor;

#if _ALPHATEST_ON
                uniform MidPrec _AlphaCutoff; // todo: 支持透贴
#endif // _ALPHATEST_ON
...
// #endif // todo: 裁剪时也支持描边（需要在这个ps中把 screenPos semantic 支持下）

                // uniform HighPrec4x4 _PreScaleWorldMat; // toon outline：model缩放以后的 object->world transform
                // _PreScaleWorldMat 相对于 unity_ObjectToWorld 而言，是在scale方面多scale了一些

                vertOutput vert(vertInput input)
                {
                    vertOutput o = (vertOutput)0;
                    o.uv = input.uv * _DiffuseTex_ST.xy + _DiffuseTex_ST.zw;

                    if(_OutlineEffect > PROPERTY_ZERO){

                            // float3 normal = any(input.smoothNormal) ? input.smoothNormal : input.nor;
                            HighPrec3 normal = input.nor;
                            ...
                            o.pos = OutlineCalcInViewSpace(modelNew, viewNew, input.pos,input.nor, input.color, _OutlineUseVertexColor, _OutlineThickness);
                            // o.pos = OutlineCalcInClipSpace(modelNew, viewNew, input.pos,input.nor, input.color, _OutlineUseVertexColor, _OutlineThickness);


                    }
                    else{
                        o.pos =  float4(0,0,1,1); //UnityObjectToClipPos(input.pos);
                    }

                    return o;
                }

                MidPrec4 frag(vertOutput input) : SV_Target
                {
                    if(_FrozenEffect > PROPERTY_ZERO)
                    {
                        clip(-1);
                        return MidPrec4(0.01h, 0.01h, 0.01h, 1);
                    }
#if _HIGH2LOWBAKE_ON || _GLOBAL_HAWKEYE_ON || _SCREENCLIPSWITCH_ON// 在高配烘低配时 / 开启阴阳眼效果时 / 冰冻时，不描边
                    clip(-1);
                    return MidPrec4(0.01h, 0.01h, 0.01h, 1);
#endif
                    ...
                    if( _OutlineEffect > PROPERTY_ZERO ){
                        #if _ALPHATEST_ON                       
                            MidPrec4 diffuseColor = tex2D(_DiffuseTex, input.uv);
                            MidPrec outputAlpha = diffuseColor.a * _Color.a;
                            clip(outputAlpha - _AlphaCutoff);
                        #endif
                        
                        if(_AnisoEffect > PROPERTY_ZERO){
                            _OutlineColor.rgb *= _OutlineLight;
                        }

                        return MidPrec4( max( MidPrec3(0.01h,0.01h,0.01h), _OutlineColor.rgb ), _OutlineColor.a ); // 直接使用 _OutlineColor，如果是(0,0,0)，可能因为apple metal的问题在ios上描边变成白色？
                    }
                    else{
                        clip(-1);
                        // return _OutlineColor;
                        return MidPrec4( max( MidPrec3(0.01h,0.01h,0.01h), _OutlineColor.rgb ), _OutlineColor.a );
                    }
        ...
        Pass {
           ...
           #pragma multi_compile _ _RENDERING_DEPTH_TEXTURE
            ...
            vertexOutput vert( vertexInput v ) {
                ...
                o.pos = Custom_UnityClipSpaceShadowCasterPos(v.vertex, v.normal, v.color, _OutlineUseVertexColor, _OutlineThickness * _OutlineEffect);
                // o.pos = UnityApplyLinearShadowBias(o.pos);

#else 
                // 投射阴影时就不再进行大世界矩阵处理和描边处理，节省GPU计算量
