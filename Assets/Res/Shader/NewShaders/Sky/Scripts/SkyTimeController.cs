...
namespace Renderering.Sky
{
    ...
    public class SkyTimeController : MonoBehaviour
    {
        ...
        [SerializeField] private bool _autoTime;
        ...
        [SerializeField] private float _minLightAltitude = 0.0f;

        [Tooltip("Will the 'time of day' be evaluated based on the timeline or based on the day-night length curve?")]
        [SerializeField] private bool _isTimeEvaluatedByCurve = false;

        [Tooltip("Duration of the day-night cycle in minutes.")]
        [SerializeField] private AnimationCurve _dayLengthCurve = AnimationCurve.Linear(0, 0, 24, 24);

        [Tooltip("List storing all celestial bodies currently in use by the system.")]
        [SerializeField] private List<CelestialBody> _celestialBodiesList = new List<CelestialBody>();

        [Tooltip("Event triggered when the minute changes.")]
        [SerializeField] private UnityEvent _onMinuteChange = new UnityEvent();

        [Tooltip("Event triggered when the hour changes.")]
        [SerializeField] private UnityEvent _onHourChange = new UnityEvent();

        [Tooltip("Event triggered when the day changes to the next day at midnight.")]
        [SerializeField] private UnityEvent _onDayChange = new UnityEvent();

        [Tooltip("The correct time of day converted from the timeline.")]
        private float _timeOfDay = 6.0f;

        [Tooltip("The time progression step used to change the time of day.")]
        private float _timeProgressionStep = 0f;

        [Tooltip("The sun elevation in the sky.")]
        private float _sunElevation = 0f;

        [Tooltip("The moon elevation in the sky.")]
        private float _moonElevation = 0f;

        [Tooltip("The distance between Earth and the Sun.")]
        private float _sunDistance = 0f;
        ...
        private float _moonDistance = 0f;

        [Tooltip("The time interval the system should use for checking the custom event list.")]
        [SerializeField] private EventScanMode _customEventScanMode = EventScanMode.ByMinute;

        [Tooltip("The list of custom events.")]
        [SerializeField] private List<CustomEvent> _customEventList = new List<CustomEvent>();

        // Calendar stuffs
        private DateTime _dateTime;
        private int _daysInMonth = 30;
        private int _previousDaysInMonth = 30;
        private int _previousMonth = 1;
        private int _dayOfWeek = 0;
        [Tooltip("String array that stores the name of each week day.")]
        private readonly string[] _weekNameList = new string[]
        {
            "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"
        };

        [Tooltip("String array that stores the name of each month.")]
        private readonly string[] _monthNameList = new string[]
        {
        ...
        [SerializeField]
        private string[] _dayNumberList = new string[]
        {
        ...
        private int _timelineTransitionTargetHour = 0;
        private int _timelineTransitionTargetMinute = 0;
        private float  _timelineTransitionTargetSpeed = 1f;
        private int _timelineTransitionTargetMonth = 0;
        private int _timelineTransitionTargetDay = 0;
        private int _timelineTransitionTargetYear = 0;
        private TimeDirection _timelineTransitionDirection = TimeDirection.Forward;
        private bool _isTimelineTransitionInProgress = false;

        // Event stufss
        private int _previousHour = 6;
        private int _previousMinute = 0;
        
        private static SkyTimeController _instance; 
        public static SkyTimeController Instance
        { 
            get
            {
                if (_instance == null)
                    _instance = GameObject.FindObjectOfType<SkyTimeController>();
        ...
        private void Awake()
        {
            _timeProgressionStep = GetTimeProgressionStep();
        ...
        private void Update()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                if (_autoTime)
                {
                    ...
                    if (_timeDirection == TimeDirection.Forward)
                    {
                        _timeline += _timeProgressionStep * Time.deltaTime;

                        if (_isTimelineTransitionInProgress)
                ...
                if (_previousMinute != _minute)
                {
                    _previousMinute = _minute;
                    _onMinuteChange?.Invoke();
                    if (_customEventScanMode == EventScanMode.ByMinute)
                        ScanCustomEventList();
                }

                // On hour change event
                if (_previousHour != _hour)
                {
                    _previousHour = _hour;
                    _onHourChange?.Invoke();
                    if (_customEventScanMode == EventScanMode.ByHour)
                        ScanCustomEventList();
                }
            }
        ...
        /// </summary>
        public void StartTimelineTransition(int hour, int minute, float speedMultiplier, TimeDirection timeDir = TimeDirection.Forward, int day = int.MaxValue, int month = int.MaxValue, int year = int.MaxValue)
        {
            _timelineTransitionTargetHour = Mathf.Min(hour, 23);
            _timelineTransitionTargetMinute = Mathf.Min(minute, 59);
            _timelineTransitionTargetSpeed = Mathf.Max(1.0f, _timeProgressionStep * speedMultiplier);
            _timelineTransitionTargetDay = day;
            _timelineTransitionTargetMonth = month;
            _timelineTransitionTargetYear = year;
            _timelineTransitionDirection = timeDir;

            if (timeDir == TimeDirection.Backward)
            {
            ...
            _isTimelineTransitionInProgress = true;
        }

        public void CancelTimelineTransition()
        {
            _isTimelineTransitionInProgress = false;
        }

        private void ApplyTimelineTransition()
        {
            switch (_timelineTransitionDirection)
            {
        ...
        {
            _timeline += _timelineTransitionTargetSpeed * Time.deltaTime;

            if (_hour >= _timelineTransitionTargetHour)
            {
                if (_minute >= _timelineTransitionTargetMinute)
                {
                    if (_timelineTransitionTargetDay == int.MaxValue && _timelineTransitionTargetMonth == int.MaxValue && _timelineTransitionTargetYear == int.MaxValue)
                    {
                        CancelTimelineTransition();
                        return;
                    }
                    
                    if (_day >= _timelineTransitionTargetDay)
                    {
                        if (_timelineTransitionTargetMonth == int.MaxValue && _timelineTransitionTargetYear == int.MaxValue)
                        {
                            CancelTimelineTransition();
                            return;
                        }

                        if (_month >= _timelineTransitionTargetMonth)
                        {
                            if (_timelineTransitionTargetYear == int.MaxValue)
                            {
                                CancelTimelineTransition();
                                return;
                            }

                            if (_year >= _timelineTransitionTargetYear)
                            {
                                CancelTimelineTransition();
                                return;
                            }
        ...
        private void ApplyTimelineTransitionBackward()
        {
            ...
            if (_hour <= _timelineTransitionTargetHour)
            {
                if (_minute <= _timelineTransitionTargetMinute)
                {
                    if (_timelineTransitionTargetDay == int.MinValue && _timelineTransitionTargetMonth == int.MinValue && _timelineTransitionTargetYear == int.MinValue)
                    {
                        CancelTimelineTransition();
                        return;
                    }
                    ...
                    if (_day <= _timelineTransitionTargetDay)
                    {
                        if (_timelineTransitionTargetMonth == int.MinValue && _timelineTransitionTargetYear == int.MinValue)
                        {
                            CancelTimelineTransition();
                            return;
                        }
                        ...
                        if (_month <= _timelineTransitionTargetMonth)
                        {
                            if (_timelineTransitionTargetYear == int.MinValue)
                            {
                                CancelTimelineTransition();
                                return;
                            }
        ...
        {
            _timeOfDay = _isTimeEvaluatedByCurve ? _dayLengthCurve.Evaluate(_timeline) : _timeline;
            _hour = (int)Mathf.Floor(_timeOfDay);
            _minute = (int)Mathf.Floor(_timeOfDay * 60 % 60);
        }
        ...
        private void EvaluateSunMoonElevation()
        {
            ...
            _moonElevation = Vector3.Dot(-_moonLocalDirection, Vector3.up);
        }
        ...
        {
            _directionalLight.transform.localRotation = Quaternion.LookRotation(_sunElevation >= 0.0f ? _sunLocalDirection : _moonLocalDirection);

            // Avoid the directional light to get close to the horizon line
            if (_minLightAltitude > 0f && _minLightAltitude < 90f)
            {
                _lightLocalDirection = _directionalLight.transform.localEulerAngles;

                if (_lightLocalDirection.x <= _minLightAltitude) _lightLocalDirection.x = _minLightAltitude;
                    _directionalLight.transform.localEulerAngles = _lightLocalDirection;
            }
        }
        ...
        //{
        //    if (_customEventList.Count <= 0)
        //        return;
        //
        //    for (int i = 0; i < _customEventList.Count; i++)
        //    {
        //        if (_customEventList[i].unityEvent == null)
        //            continue;
        //        if (_customEventList[i].year != _year && _customEventList[i].year != -1)
        //            continue;
        //        if (_customEventList[i].month != _month && _customEventList[i].month != -1)
        //            continue;
        //        if (_customEventList[i].day != _day && _customEventList[i].day != -1)
        //            continue;
        //        if (_customEventList[i].hour != _hour && _customEventList[i].hour != -1)
        //            continue;
        //        if (_customEventList[i].minute != _minute && _customEventList[i].minute != -1)
        //            continue;
        //        _customEventList[i].unityEvent.Invoke();
        //    }
        //}
        ...
        private void ScanCustomEventList()
        {
            ...
            for (int i = 0; i < _customEventList.Count; i++)
            {
                ...
                if (_hour != _customEventList[i].executedHour) _customEventList[i].isAlreadyExecutedOnThisHour = false;
                if (_customEventList[i].hour != _hour && _customEventList[i].hour != -1)
                    continue;

                if (_customEventList[i].minute == -1)
                {
                    _customEventList[i].unityEvent.Invoke();
                }
                else
                {
                    if (!_customEventList[i].isAlreadyExecutedOnThisHour)
                    {
                        if (_minute > _customEventList[i].minute)
                        {
                            _customEventList[i].executedHour = _hour;
                            _customEventList[i].isAlreadyExecutedOnThisHour = true;
                            _customEventList[i].unityEvent.Invoke();
                        }
                    }
                }
        ...
        private void ComputeCelestialCoordinates()
        {
            switch (_timeSystem)
            {
                case TimeSystem.Simple:
                    _sunTransform.localRotation = GetSunSimpleRotation();
                    _moonTransform.localRotation = _sunTransform.localRotation * Quaternion.Euler(0, -180, 0);
                    Shader.SetGlobalMatrix(SkyShaderUniforms.StarFieldMatrix, _sunTransform.worldToLocalMatrix);
                    break;

                case TimeSystem.Realistic:

                    // Initializations
                    float hour = _timeOfDay - _utc;
                    float rad = Mathf.Deg2Rad;
                    float deg = Mathf.Rad2Deg;
                    float latitude = _latitude * rad;

                    // The time scale
                    float d = 367 * _year - 7 * (_year + (_month + 9) / 12) / 4 + 275 * _month / 9 + _day - 730530;
                    d = d + (hour / 24.0f);
                    ...
                    _sunTransform.localRotation = Quaternion.Euler(celestialRotation);
                    _sunDistance = r * 150000000f;
                    Shader.SetGlobalMatrix(SkyShaderUniforms.StarFieldMatrix,
                    Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(90.0f - _latitude, 0.0f, 0.0f) * Quaternion.Euler(0.0f, _longitude, 0.0f) * Quaternion.Euler(0.0f, LST * deg, 0.0f), Vector3.one).inverse);

                    // Applys position to the sun
                    //Quaternion direction = Quaternion.Euler(celestialRotation);
                    //Vector3 pos = direction * new Vector3(0, 0, -_astronomicalUnit);
                    //_sunTransform.transform.localPosition = pos;
                    ...
                    celestialRotation.y = azimuth * deg;
                    celestialRotation.z = 0.0f;
                    _moonTransform.localRotation = Quaternion.Euler(celestialRotation);
                    _moonDistance = r * 6371f;

                    // Planets
                    if (_celestialBodiesList.Count > 0)
                    {
                        CelestialBody celestialBody;
                        for (int index = 0; index < _celestialBodiesList.Count; index++)
                        {
                            celestialBody = _celestialBodiesList[index];
                            if (!celestialBody.transform) continue;

                            // Orbital elements of the planets
                            switch (celestialBody.type)
                            {
                                case CelestialBody.Type.Mercury:
                                ...
                                case CelestialBody.Type.Jupiter:
                                ...
                                case CelestialBody.Type.Saturn:
                                ...
                                case CelestialBody.Type.Uranus:
                                ...
                                case CelestialBody.Type.Neptune:
        ...
        {
            _dateTime = new DateTime(_year, _month, _day);
            return _weekNameList[(int)_dateTime.DayOfWeek];
        }
        ...
        public void UpdateCalendar()
        {
            ...
            _day = Mathf.Clamp(_day, 1, _daysInMonth);
            _month = Mathf.Clamp(_month, 1, 12);
            _year = Mathf.Clamp(_year, 0, 9999);
            ...
            _dayOfWeek = (int) _dateTime.DayOfWeek;

            // Keeps the same day selected in the calendar even when the date is changed externally.
            _selectedCalendarDay = _day - 1 + _dayOfWeek;
            
            for (int i = 0; i < _dayNumberList.Length; i++)
            {
                // Make null all the calendar buttons
                if (i < _dayOfWeek || i >= (_dayOfWeek + _daysInMonth))
                {
                    _dayNumberList[i] = "";
                    continue;
                }
                ...
                _dateTime = new DateTime(_year, _month, (i - _dayOfWeek) + 1);
                _dayNumberList[i] = _dateTime.Day.ToString();
            }
        }
        ...
        public void SetUTC(float value)
        {
            _utc = value;
        }
        ...
        {
            return new Vector2(_hour, _minute);
        }
        ...
        /// </summary>
        public float GetEvaluateTime() { return _timeOfDay; }

        /// <summary>
        /// Returns the sun elevation in the sky.
        /// </summary>
        public float GetSunElevation() { return _sunElevation; }

        /// <summary>
        /// Returns the moon elevation in the sky.
        /// </summary>
        public float GetMoonElevation() { return _moonElevation; }
        ...
        {
            return new Vector3Int(_year, _month, _day);
        }

        /// <summary>
        /// Returns the current date converted to string using the default format used by .
        /// </summary>
        public string GetDateString()
        {
            // Format: "MMMM dd, yyyy"
            return _monthNameList[_month - 1] + " " + _day.ToString("00") + ", " + _year.ToString("0000");
        }
        ...
        public void DecreaseDay()
        {
            ...
            {
                _day--;
                _previousMonth = _dateLoop == DateLoop.ByMonth ? _month : _month - 1;
                if (_previousMonth < 1) _previousMonth = 12;
                _previousDaysInMonth = DateTime.DaysInMonth(_year, _previousMonth);

                if (_day < 1)
                {
                    _day = _previousDaysInMonth;
                    DecreaseMonth();
                }
            }
        ...
        {
            return Quaternion.Euler(0.0f, _longitude, -_latitude) * Quaternion.Euler(((_timeOfDay + _utc) * 360.0f / 24.0f) - 90.0f, 180.0f, 0.0f);
        }
