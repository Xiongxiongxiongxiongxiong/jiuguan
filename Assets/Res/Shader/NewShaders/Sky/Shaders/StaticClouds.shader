Shader "Hidden/MT/Buintin/StaticClouds"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            uniform float  _Azure_StaticCloudSaturation;
            ...
            float4 fragment_program (Varyings Input) : SV_Target
            {
                ...
                float moonMask = 1.0 - moonTexture.a * _Azure_MoonTextureIntensity;
                ...
                float3 lowCloudColor = lerp(inScatter * _Azure_StaticCloudScattering * cloudExtinction, _Azure_StaticCloudColor, lowCloudTex.r * pow(fex.r, _Azure_StaticCloudExtinction)) * _Azure_StaticCloudOpacity;
                float3 higCloudColor = lerp(inScatter * _Azure_StaticCloudScattering * cloudExtinction, _Azure_StaticCloudColor, higCloudTex.r * pow(fex.r, _Azure_StaticCloudExtinction)) * _Azure_StaticCloudOpacity;
                lowCloudColor = pow(lowCloudColor, _Azure_StaticCloudSaturation);
                higCloudColor = pow(higCloudColor, _Azure_StaticCloudSaturation);
                float3 cloud = lerp(lowCloudColor, higCloudColor, higCloudAlpha);
                float cloudMask = (1.0 - higCloudTex.g) * lowCloudAlpha;

                // Output
                float3 OutputColor = inScatter + skyLuminance + cloud + (sunTexture + moonColor + starfield) * lerp(1.0, cloudMask, saturate(_Azure_StaticCloudOpacity));
