Shader "MT/Builtin/StylizedWater_Mask"
{
    Properties
    {
        [Header(Densities)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                o.maskUV = TRANSFORM_TEX(v.uv,_MaskMap);
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float3 color = baseColor + reflectedColor + sssColor + causticColor + sunSpecularColor + foamColor + edgeFoam + cubeMap;
                
                //Mask
                float mask = tex2D(_MaskMap,i.maskUV);
                color *= mask;

                UNITY_APPLY_FOG(i.fogCoord, color);
                return float4(color, _Alpha*mask);
            }
            ENDCG
        }
    }
