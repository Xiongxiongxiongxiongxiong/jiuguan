Shader "MT/MT_VFX_CullOff_VertexOffset"
{
    Properties
    {
        //        [Enum(UnityEngine.Rendering.BlendMode)]_Dest ("Alpha/ADD", int) = 0
        ...
        [Header(VertexOffset)]
        [Toggle]_VertexOffset("VertexOffset",int) = 0
        _VertexMap("VertexMap R",2D) = "black"{}
        _MaskMap("MaskMap R",2D) = "white"{}
        _VertexMap_USpeed("Vertex U Speed",float) = 0
        _VertexMap_VSpeed("Vertex V Speed",float) = 0
        _MaskMap_USpeed("Mask U Speed",float) = 0
        _MaskMap_VSpeed("Mask V Speed",float) = 0
        _MaxOffsetDistance("MaxOffstDistance(Meter)",range(-10,10)) = 0

        [Header(Gradual)]
        [Toggle]_Ramp("Ramp",int) = 0
        [Toggle]_Dir("RampColorDirection false:Hor true:Ver",int) = 0
        [NoScaleOffset]_RampColor("RampColor",2D) = "white"{}
        _RampMask("RampMask",2D) = "white"{}
        _RampMask_USpeed("RampMask U Speed",float) = 0
        _RampMask_VSpeed("RampMask V Speed",float) = 0
        _RampRotate("Ramp Rotate",range(0,360)) = 0
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            #pragma shader_feature _VERTEXOFFSET_ON
            ...
            sampler2D _MaskMap;
            float4 _MaskMap_ST;
            float _MaxOffsetDistance;
            float _VertexMap_USpeed;
            float _VertexMap_VSpeed;
            float _MaskMap_USpeed;
            float _MaskMap_VSpeed;

            sampler2D _RampColor;
            sampler2D _RampMask;
            float4 _RampMask_ST;
            float _RampRotate;
            float _RampMask_USpeed;
            float _RampMask_VSpeed;
            ...
            float2 DoDistort(float2 uv, float FactorOffset)
            {
                ...
                float2 Distort = tex2D(_DistortTex, DistortUV) * (_DistortFactor + FactorOffset);
            ...
            float DoDIsslove(float2 uv, float FactorOffset)
            {
                ...
                DIsslove -= (_DIssloveFactor + FactorOffset) * 2;
            ...
            v2f vert(appdata v)
            {
                ...
                half2 vertexUV = TRANSFORM_TEX(v.uv.xy, _VertexMap);
                half2 maskUV = TRANSFORM_TEX(v.uv.xy, _MaskMap);
                half offsetMap = tex2Dlod(_VertexMap, float4(vertexUV + _Time.y * half2(_VertexMap_USpeed, _VertexMap_VSpeed), 0, 0)).r;
                half maskMap = tex2Dlod(_MaskMap, float4(maskUV + _Time.y * half2(_MaskMap_USpeed, _MaskMap_VSpeed), 0, 0)).r;
                v.vertex.y += offsetMap * maskMap * _MaxOffsetDistance;
                #endif
                ...
                #ifdef _RAMP_ON
                o.rampMaskUV = TRANSFORM_TEX(v.uv, _RampMask);
                #endif
            ...
            {
                // sample the texture
                float2 mainUV = GetUV(i.uv.xy + i.uv.zw, _MainTex_ST, _MainTex_Uspeed, _MainTex_Vspeed);


                float2 maskUV = GetUV(i.uv.xy + i.texcoord1.xy, _MaskTex_ST, _MaskTex_Uspeed, _MaskTex_Vspeed);
                float mask = tex2D(_MaskTex, maskUV).r;
                #ifdef _DISTORTON_ON
                mainUV += DoDistort(i.uv.xy, i.texcoord1.z);
                #endif
                ...
                col *= i.vertexColor;
                #ifdef _DISSLOVEON_ON
                float disslove = DoDIsslove(i.uv.xy, i.texcoord1.w);
...
#ifdef _RAMP_ON
                _RampRotate = _RampRotate * 3.1415926 / 180;
                half2x2 Rot = half2x2(cos(_RampRotate), -sin(_RampRotate), sin(_RampRotate), cos(_RampRotate));
                half rampMask = tex2D(_RampMask, mul(Rot, i.rampMaskUV +_Time.y *float2(_RampMask_USpeed,_RampMask_VSpeed) - 0.5) + 0.5).r;
                #ifndef _DIR_ON
                half4 ramp = tex2D(_RampColor,half2(rampMask,1));
                col.rgb *= ramp.rgb;
                #else
                half4 ramp = tex2D(_RampColor, half2(1, rampMask));
                col.rgb *= ramp.rgb;
                #endif
#endif
